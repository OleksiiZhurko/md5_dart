...
class Command {
  ...
  void execute() {
    ...
    while ((s = _enterCommand()) != '\\exit') {
      if (s.startsWith('\\path ')) {
        try {
          _printOutput(MD5.getHash(
              base64.encode(File(s.substring(s.lastIndexOf(' ') + 1))
                  .readAsBytesSync())));
        } on FileSystemException {
          _printOutput('The given file doesn\'t exists');
        }
      } else {
  ...
  void _printer([String s = '', String symbol = '\$']) => stdout.write('$symbol$s');
